...
class SubmissionStatus(Base):
    ...
    submission_id = Column(Integer, ForeignKey("student_submissions.submission_id"), index=True)
    student_id = Column(Integer, ForeignKey("students.student_id"), index=True)
    lecturer_id = Column(Integer, ForeignKey("lecturers.lecturer_id"), index=True)
